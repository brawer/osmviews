...
// sentinel string standing in for the GeoTIFF it describes, so the values are
// valid-length hex and stable without pretending to be digests of a real file.
...
func TestDataPURL(t *testing.T) {
	...
	want := "pkg:generic/osmviews@2025-08-30?checksum=sha256:" + in.SHA256 +
		"&download_url=https%3A%2F%2Fosmviews.brawer.ch%2Fdata%2Fosmviews-20250830.tiff"
...
// TestBOM_NoStatsReference guards that the retired sidecar statistics JSON is
// no longer referenced now that the histogram lives in the GeoTIFF itself.
func TestBOM_NoStatsReference(t *testing.T) {
	for _, ref := range buildBOM(bomTestInputs()).Metadata.Component.ExternalReferences {
		if strings.Contains(ref.URL, "osmviews-stats-") {
			t.Errorf("GeoTIFF still references a stats sidecar: %+v", ref)
		}
	}
}
